...
def batch_gradient_off_policy(value_function, target_policy, data, lambda_param, discount_factor, return_type, num_episodes,
...
def batch_AB_Trace(value_function, target_policy, behavior_policy, data, lambda_param, discount_factor, num_episodes,
                   obj_function, alpha_0):
    nS = value_function.nS
    nA = value_function.nA
    ...
    for i, episode in enumerate(data):
        ...
        for idx, (state, action, next_state, reward, behavior_prob, target_prob) in enumerate(
            ...
            if i % 20 == 0:
                alpha_omega = alpha_0 / np.sqrt(i + 1)
                alpha_theta = alpha_0 / (i + 1)
            old_omega = omega.copy()
            old_theta = theta.copy()
            ...
            if idx == len(episode['states']) - 1:
                ...
                weighted_phiprime = np.sum(
                    [min(1, target_policy[next_state, a]/behavior_policy[next_state, a]) * value_function.feature(next_state, a)
                     for a in np.arange(nA)], axis=0)
